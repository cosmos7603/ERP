...
{
    public class ProviderManager : ManagerBase<ProviderPOCO, Provider>
    {
        ...
        public override bool Update(ProviderPOCO poco, int id)
        {
            try
            {
                using (var unitOfWork = new UnitOfWork<Provider>())
                {
                    var provider = unitOfWork.Repository.GetById(id);
                    Mapper.Map(poco, provider);
                    unitOfWork.Save();
                }
                return true;
            }
