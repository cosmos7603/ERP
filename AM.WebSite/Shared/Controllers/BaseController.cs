...
namespace AM.WebSite.Shared.Controllers
{
	public class BaseController : Controller
    {
        ...
        protected string ViewPath(string viewName)
        {
            ...
            // No attribute?
            if (this.GetType().GetCustomAttributes(typeof(ViewsPath), false).Count() == 0)
                return viewName;

            // Get view path and return based on that
            var attribute = (ViewsPath)this.GetType().GetCustomAttributes(typeof(ViewsPath), false).First();
