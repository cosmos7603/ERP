...
namespace AM.Utils.FeedParser
{
	...
	public class FeedParser
	{
		...
		public virtual IList<Item> ParseAtom(string url)
		{
			...
			// Feed/Entry
			var entries = doc.Root.Elements().Where(i => i.Name.LocalName == "entry").Select(x => new Item
			{
				FeedType = FeedType.Atom,
				Content = x.Elements().First(i => i.Name.LocalName == "summary").Value,
				Link = x.Elements().First(i => i.Name.LocalName == "link" && i.Attribute("rel").Value == "alternate").Attribute("href").Value,
				PublishDate = ParseDate(x.Elements().First(i => i.Name.LocalName == "published").Value),
